...
function Register() {
  ...
  function handleSubmit(e) {
    ...
    dispatch(
      updateUserField({ field: "firstname", value: dataObject.firstname })
    );
    dispatch(
      updateUserField({ field: "lastname", value: dataObject.lastname })
    );
    dispatch(
      updateUserField({ field: "user_type", value: dataObject.user_type })
    );
    ...
    fetch("https://class-schedule-pp4h.onrender.com/signup", {
      ...
      .then((r) => {
        if (r.ok) {
          ...
          throw new Error(
            `Oops! Invalid details or try again later! : ${r.status}`
          );
        }
      })
      .then((responseData) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen dark">
      <div className="some w-full max-w-md rounded-lg shadow-md p-6">
        <p className="title hover:scale-110 transition-all duration-500">
          Register
        </p>
        ...
        <form className="flex flex-col" onSubmit={handleSubmit}>
          ...
          {dataObject.error && (
            <p className="text-red-500">{dataObject.error}</p>
          )}{" "}
          {/* Display error message */}
          <button
